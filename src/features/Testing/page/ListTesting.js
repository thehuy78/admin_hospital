...
export default function ListTesting() {
  ...
  const changeStatus = async (id) => {
    try {
      if (token) {
        var rs = await apiRequestAutherize("GET", `testing/changestatus/${id}`, token)
        if (rs && rs.data && rs.data.status === 200) {
          fetchdata()
  ...
  const hanldeUpdate = async (e) => {
    try {
      ...
      if (token) {
        ...
        var rs = await apiRequestAutherize("post", "testing/update", token, data)
        console.log(rs);
        if (rs && rs.data && rs.data.status === 200) {
          setMocupEdit(false)
          setValueForm({
          ...
          fetchdata()
          swal("Update", "Update Testing Success!", "success");
        } else {
  ...
  const handleCreate = async (e) => {
    try {
      ...
      if (token) {
        ...
        var rs = await apiRequestAutherize("post", "testing/create", token, data)
        console.log(rs);
        if (rs && rs.data && rs.data.status === 200) {
          ...
          fetchdata()
          swal("Create", "Create Testing Success!", "success");
        } else {
          if (rs?.data?.message === "Duplicate code") {
            setErrorForm((prev) => ({
              ...prev,
              ...
              {testing &&
                ...
                testing.map((item, index) => (
                  <tr key={index}>
                    ...
                    <td>
                      <div className='list_action'>
                        <Link className='link_tag' to={`/admin/booking/testing/${item.code}`}><i data-tooltip-id="tooltip"
      ...
      {mocupEdit && (
        <div className='mocup_edit_box'>
          <div className='container'>
            ...
            <p className='title'>Edit Testing</p>
      ...
      {mocupCreate && (
        <div className='mocup_edit_box'>
          <div className='container'>
            ...
            <p className='title'>Create New Testing</p>
